...
- `Publish agent-session-store` publishes `@botiverse/agent-session-store` to npm with the repository npm token. Trigger it manually with the package version from `packages/agent-session-store/package.json`, or push a tag like `session-store-v0.1.0`.
- `Publish CLI` publishes `@botiverse/hands-cli` with the repository npm token. Publish the package's declared `@botiverse/hands-node` and `@botiverse/agent-session-store` versions first; the workflow verifies they exist, packs with pnpm so the workspace ranges become normal npm semver ranges, and then publishes the tarball. Trigger it manually with the package version from `packages/cli/package.json`, or push a tag like `cli-v0.5.1`.
