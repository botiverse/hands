...
function UploadDialog({
  ...
  const parse = useMutation({
    mutationFn: async (f: File) => {
      const m = await parseApk(f);
      return { metadata: m, file: f };
    },
    onSuccess: ({ metadata }) => setMetadata(metadata),
    onError: (e) => setError((e as Error).message),
  });
  ...
  const submit = useMutation({
    mutationFn: () => {
      ...
      return createVersion(appId, {
        ...
        r2_key: `apps/${appId}/pending/${metadata.file_hash_sha256}.apk`,
      });
    },
    onSuccess: onCreated,
    onError: (e) => setError((e as Error).message),
      ...
      <div className="card max-w-lg w-full">
        ...
        {!metadata ? (
          <div className="space-y-3">
            ...
            />
            {parse.isPending && (
              <p className="text-slate-500 text-sm">Parsing APK metadata via container...</p>
            )}
            {error && <p className="text-red-600 text-sm">{error}</p>}
          </div>
        ) : (
