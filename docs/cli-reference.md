...
> **Caveat (updated 2026-07-04)**: `@oranix/quiver-cli` is published on npm as
...
- **Phase 3.4 (CLI npm package)** — 🟡 ALPHA: `packages/cli/` is published as public npm package `@oranix/quiver-cli@0.1.1`. Current published binary covers auth, app/build listing, Android release publishing via `quiver builds publish-android`, and basic release share management via `quiver releases`. The source package version `0.1.2` adds `quiver releases update-share` for share renewal, but npm publish requires an authenticated npm session. Ops/webhooks commands remain planned.
