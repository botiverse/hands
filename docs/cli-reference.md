...
- One-off execution: `npm exec --package @oranix/quiver-cli@0.1.2 -- quiver --help`
...
- **Phase 3.4 (CLI npm package)** — 🟡 ALPHA: `packages/cli/` is published as public npm package `@oranix/quiver-cli@0.1.2`. Current binary covers auth, app/build listing, Android release publishing via `quiver builds publish-android`, and release share management via `quiver releases`. Ops/webhooks commands remain planned.
...
# or one-off
npm exec --package @oranix/quiver-cli@0.1.2 -- quiver --help
```
