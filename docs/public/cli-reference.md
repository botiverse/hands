...
### Local macOS notarization with Hands credential brokerage

Hands can keep one encrypted App Store Connect team key per app without copying
it into repository secrets. Configure the key in the app's Hands settings,
give a protected macOS release job an app-scoped `publisher` deploy token, and
notarize the already Developer-ID-signed local artifact:
...
The command computes the pre-staple SHA-256 and size, asks Hands for a
non-cacheable audited export of that app's key, writes it into a mode-0600
temporary directory, and invokes `xcrun notarytool submit --wait` directly on
the local file. It then downloads Apple's developer log, requires its SHA-256
to equal the pre-staple local digest, and runs `stapler staple` plus `stapler
validate`. The temporary credential directory is removed on every exit path.

Hands never receives or stores the artifact, so there is no R2 intermediary.
The exported team key remains sensitive and usable until revoked at Apple;
only use this command in a protected ephemeral runner. `--json` contains only
the non-secret export receipt, Apple submission id, pre-staple binding, and
final stapled digest. It never contains the private key or Apple developer log.
...
macOS update artifacts must be Developer-ID-signed before local notarization,
then stapled and validated before `publish-electron`; Hands hosts only those
final bytes and does not sign Electron applications.
