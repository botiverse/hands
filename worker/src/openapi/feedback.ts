...
export function registerFeedbackRoutes(registry: OpenApiRegistry) {
  ...
  register(registry, {
    ...
    description: "Idempotently moves only the authenticated reporter's ticket to the closed state. This route cannot reopen tickets, choose another status, or change assignees.",
    security: auth,
  ...
  register(registry, {
    method: "post",
    path: "/api/apps/{appId}/reporter-feedback/{ticketId}/reopen",
    tags: ["Reporter Feedback"],
    summary: "Reopen a reporter-owned feedback ticket",
    description: "Idempotently moves only the authenticated reporter's ticket to the open state. This route cannot choose another status or change assignees.",
    security: auth,
    request: { params: AppTicketParams, headers: ReporterHeaders },
    responses: {
      200: success("Ticket reopened or already open.", GenericObject),
      400: error("Missing or malformed reporter id."),
      401: error("Missing or invalid bearer token."),
      403: error("Invalid reporter integration grant."),
      404: error("Ticket is not owned by this reporter integration."),
